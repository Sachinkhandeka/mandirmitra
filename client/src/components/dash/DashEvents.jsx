...
export default function DashEvents() {
    ...
    const hasPermission = (actions) => {
        return currUser.isAdmin || (currUser.roles && currUser.roles.some(role =>
            role.permissions.some(p => actions.includes(p.actions))
        ));
    };
    ...
    return (
        <>
            ...
            {events && events.length > 0 ? (
                <>
                    {hasPermission(["read", "update", "delete"]) && (
